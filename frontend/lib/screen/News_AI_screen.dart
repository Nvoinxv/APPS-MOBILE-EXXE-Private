...
import './ai_news_detail_screen.dart'; // ← NEW IMPORT
...
class _NewsScreenState extends State<NewsScreen> {
  ...
  Future<void> _triggerAiBackground() async {
    setState(() { _aiGenerating = true; _aiError = null; });
    try {
      ...
      if (hook.error != null) { _aiError = hook.error!.message; return; }
  ...
  // ← NEW: buka detail AI news
  void _openAiNews(UnifiedNewsItem item) {
    if (item.aiData == null) return;
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AiNewsDetailScreen(
          aiNewsData: item.aiData!.toJson(),
        ),
      ),
    );
  }
          ...
          itemBuilder: (context, index) {
            ...
            return item.source == NewsSource.database
                ...
                : _AiNewsCard(
                    index: index,
                    item:  item,
                    onTap: () => _openAiNews(item), // ← NOW TAPPABLE
                  );
          },
        ),
      ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Loading news feed...',
            style: TextStyle(color: NewsColorStyle.subtitleText, fontSize: 14),
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: const TextStyle(color: NewsColorStyle.sourceText, fontSize: 14),
          ),
        ],
      ),
    );
...
// ===========================================================================
// _AiNewsCard — UPDATED: sekarang punya onTap + GestureDetector
// ===========================================================================
...
class _AiNewsCardState extends State<_AiNewsCard> {
  ...
  Color get _sentimentColor {
    switch (widget.item.sentiment.toLowerCase()) {
      case 'optimis': case 'positif': return const Color(0xFF00E5A0);
              ...
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: _aiAccent.withOpacity(0.4), width: 1),
                      ),
                      child: Row(
                        ...
                        children: [
                          ...
                          Text('AI NEWS', style: TextStyle(
                            color: _aiAccent, fontSize: 10,
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 5),
                            Text(item.sentiment.toUpperCase(), style: TextStyle(
                              color: _sentimentColor, fontSize: 9,
                  ...
                  Row(
                    children: [
                      ...
                      Text('${(item.confidence * 100).toStringAsFixed(0)}%',
                        style: TextStyle(
                          color: _sentimentColor, fontSize: 10,
                          fontWeight: FontWeight.w600,
                        )),
                    ],
                  ),
                ...
                Row(
                  children: [
                    ...
                    // Tap hint arrow — baru di sini
                    const SizedBox(width: 8),
                    Icon(
                      Icons.arrow_forward_ios_rounded,
                      size:  11,
                      color: _aiAccent.withOpacity(isHovered ? 0.8 : 0.35),
                    ),
                  ],
                ),
                ...
                if (item.originalDomain.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Text('# ${item.originalDomain}',
                      style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFF7B61FF).withOpacity(0.3), width: 1),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          SizedBox(
            width: 10, height: 10,
            child: CircularProgressIndicator(
              strokeWidth: 1.5, color: const Color(0xFF7B61FF),
            ),
          ),
          const SizedBox(width: 6),
          const Text('AI generating...', style: TextStyle(
            color: Color(0xFF7B61FF), fontSize: 11, fontWeight: FontWeight.w500,
          )),
        ],
      ),
    );
